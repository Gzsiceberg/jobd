...
`GET /queues/:name/workers` (admin only) returns `{ "workers": [...] }` for online workers only (last heartbeat less than two minutes ago), including idle, busy, and paused workers, ordered by hostname and worker ID. Each worker includes nullable `token_expired_time`, the UTC expiry of the token used for its latest successful heartbeat. Only verified token claims supply this value; tokens themselves are not stored. Existing workers have `null` until their next token-authenticated heartbeat. Admin operations do not overwrite recorded expiry. Offline workers are hidden, not deleted; the CLI displays expiry as a relative duration.
