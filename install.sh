...
usage() {
    printf '%s\n' 'Usage: sh install.sh [--version vX.Y.Z] [--bin-dir DIR]' \
        ...
        'Requires gh authentication with read access to the private repository.'
}
while [ "$#" -gt 0 ]; do
    case "$1" in
        --version|--bin-dir)
...
for tool in gh tar sha256sum mktemp awk grep; do
...
printf '%s\n' 'No worker was started. Restart any existing worker to use the new binary.' \
