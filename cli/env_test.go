...
func TestQueueEnvironmentCLI(t *testing.T) {
	...
	if err := runEnv(c, "set", []string{"API_KEY"}, strings.NewReader("secret\n"), &out); err != nil {
		t.Fatal(err)
	}
	if method != "PUT" || path != "/queues/batch/env/API_KEY" || value != "secret\n" || out.Len() != 0 {
		t.Fatal("incorrect secret update")
	}
	if err := runEnv(c, "list", nil, nil, &out); err != nil {
		t.Fatal(err)
	}
	if out.String() != "API_KEY\n" {
		t.Fatal("listing must contain names only")
	}
	if err := runEnv(c, "delete", []string{"API_KEY"}, nil, &out); err != nil {
		t.Fatal(err)
	}
	if method != "DELETE" {
		t.Fatal(method)
	}
	for _, args := range [][]string{{"BAD=NAME"}, {"API_KEY", "secret"}} {
		if err := runEnv(c, "set", args, strings.NewReader("v"), &out); err == nil {
			t.Fatal("invalid arguments accepted")
		}
	}
	for _, value := range []string{"x\x00y", strings.Repeat("x", 4097), "\xff"} {
		if err := runEnv(c, "set", []string{"KEY"}, strings.NewReader(value), &out); err == nil {
			t.Fatal("invalid value accepted")
		}
	}
	if err := runEnv(c, "set", []string{"JOBD_CUSTOM"}, strings.NewReader("allowed"), &out); err != nil {
		t.Fatal(err)
	}
	...
	if err := runEnv(c, "set", []string{"KEY"}, nil, &out); err == nil {
...
func TestSecretErrorsDoNotEchoResponse(t *testing.T) {
	...
	err := runEnv(c, "set", []string{"KEY"}, strings.NewReader("echoed-sensitive-value"), io.Discard)
