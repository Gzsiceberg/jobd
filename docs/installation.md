...
- `JOBD_WORKER_TOKEN`: expiring, queue-scoped worker token generated by an administrator using `jobd auth create-worker-key --duration 24h`. Without it, workers run only local jobs. Never install `JOBD_MASTER_KEY` on worker hosts.
