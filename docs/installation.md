...
- `JOBD_WORKER_TOKEN`: expiring, queue-scoped worker token generated by an administrator using `jobd auth create-worker-token --duration 24h`. Without it, workers run only local jobs. Never install `JOBD_MASTER_KEY` on worker hosts.
...
The worker inherits the CLI's environment and working directory, except the CLI strips `JOBD_MASTER_KEY` before launch. No key file is written. It survives SSH disconnects. After a crash or reboot, run a local command or `jobd worker restart` again. For automatic recovery, use an external supervisor.
