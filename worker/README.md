...
Submit with `jobd --local COMMAND...` on the same machine and as the same user. Set the CLI's `JOBD_STATE_DIR` to match the worker. The worker uses a private SQLite `:memory:` database by default. Set `JOBD_LOCAL_PERSIST=true` in the worker environment to store commands and results (not API keys) in `<state-dir>/local/queue.db` instead. `false` (the default) or `0` selects memory; `true` or `1` enables persistence. Invalid values are rejected. One database connection serializes access; SQLite transactions make claims and queue edits atomic. The CGO-free driver keeps the worker self-contained on Linux amd64/arm64. The CLI reuses its HTTP client over the worker's `<state-dir>/local/control.sock` Unix-domain socket (mode 0600, parent directory 0700). There is no TCP listener, no API key is sent, and the CLI never opens the queue files. Job progress still uses its separate NDJSON socket. Local CLI operations require the worker to be running.
...
Each local job runs once to completion before another controller claim. Heartbeats continue, but controller jobs arriving meanwhile wait. In memory mode, restart discards pending jobs and job history; local numeric IDs may be reused. In persistent mode, pending jobs survive restart and previously running jobs are marked failed rather than replayed. Local commands use the worker's working directory and environment and do not inherit `JOBD_API_KEY`. They share the same `Job` model, executor, progress reporter and cancellation lifecycle as controller jobs, but save results locally instead of reporting to the controller.

To enable persistence for the installed service, export `JOBD_LOCAL_PERSIST=true` and run `jobd --restart`. Mode changes take effect on restart; no jobs are migrated between modes. Memory mode leaves any existing `queue.db` untouched and ignores its jobs. Re-enabling persistence loads that saved queue. Output logs, worker identity, and the daemon lock remain on disk in both modes. Stop the worker before copying a persistent database for backup.
